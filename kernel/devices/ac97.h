...
private:
    ...
    void write_mixer_reg(AudioMixerRegisters reg, u16 value) { io::write<u16>(m_audio_mixer_port + reg, value); }
    u16 read_mixer_reg(AudioMixerRegisters reg) { return io::read<u16>(m_audio_mixer_port + reg); }

    void write_bus_reg16(AudioBusRegisters reg, u16 value) { io::write<u16>(m_audio_bus_port + reg, value); }
    u16 read_bus_reg16(AudioBusRegisters reg) { return io::read<u16>(m_audio_bus_port + reg); }

    void write_bus_reg32(AudioBusRegisters reg, u32 value) { io::write<u32>(m_audio_bus_port + reg, value); }
    u32 read_bus_reg32(AudioBusRegisters reg) { return io::read<u32>(m_audio_bus_port + reg); }

    void write_output_reg8(AudioRegisterBoxRegisters reg, u8 value) { io::write<u8>(m_audio_output_port + reg, value); }
    u8 read_output_reg8(AudioRegisterBoxRegisters reg) { return io::read<u8>(m_audio_output_port + reg); }

    void write_output_reg16(AudioRegisterBoxRegisters reg, u16 value) { io::write<u16>(m_audio_output_port + reg, value); }
    u16 read_output_reg16(AudioRegisterBoxRegisters reg) { return io::read<u16>(m_audio_output_port + reg); }

    void write_output_reg32(AudioRegisterBoxRegisters reg, u32 value) { io::write<u32>(m_audio_output_port + reg, value); }
    u32 read_output_reg32(AudioRegisterBoxRegisters reg) { return io::read<u32>(m_audio_output_port + reg); }

    u16 m_audio_mixer_port;
    u16 m_audio_bus_port;
    u16 m_audio_output_port;
